...
class CSSIconGeneratorController
{
    ...
    public function __invoke(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
    {
        ...
        try {
            ...
            $fontStretch = $this->parseStretchParameter($requestParams['stretch'] ?? null);
    ...
    /**
     * Checks `stretch` request query value.
     *
     * @param mixed $fontStretch Parameter value
     * @return string Valid font-stretch css value, or empty string, if $fontStretch is null or empty.
     * @throws \InvalidArgumentException If the parameter set, but has not valid value. The message may be sent back to
     *  the client.
     */
    protected function parseStretchParameter($fontStretch): string
    {
        if ($fontStretch === null) {
            return '';
        }
        if (!is_string($fontStretch)) {
            throw new \InvalidArgumentException('Invalid font stretch value');
        }
        return $fontStretch;
    }
